...
static void handleBothButtons(uint8_t ep) {
  ...
  while (pinRead(BTN1_PIN) == LOW && pinRead(BTN2_PIN) == LOW) {
    ...
    if (millis() - t_down >= LONG_MS) {
      ...
      while (pinRead(BTN1_PIN) == LOW || pinRead(BTN2_PIN) == LOW) {
        Zigbee.run();
        if (millis() - t_hold_start >= 30000) break; // Safety timeout
        delay(10);
      }
      cmd_level_stop(ep);
  ...
  // Released before LONG_MS → simple click → toggle
  cmd_toggle(ep);
}
...
static bool handleClickAndHold(uint8_t ep, BtnState *bx) {
  ...
  while (pinRead(bx->pin) == LOW) {
    ...
    if (millis() - t_hold >= LONG_MS) {
      ...
      while (pinRead(bx->pin) == LOW) {
        Zigbee.run();
        if (millis() - t_ct_start >= 30000) break; // Safety timeout
        delay(10);
      }
      cmd_ct_stop(ep);
...
void handleTarget(uint8_t ep) {
  if (ep == EP_L3) {
    handleBothButtons(ep);
  } else {
    handleSingleButton(ep);
  }
}
...
void goto_sleep() {
  ...
  uint32_t t0 = millis();
  while (millis() - t0 < 50) { Zigbee.run(); delay(5); }
